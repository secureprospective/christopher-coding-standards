...
## 2026-08-03 — PR #27/#28/#29 (Python/Bash/Go `verify-templates-*.yml` CI jobs)

**Agents involved:** Claude (Builder, merges + re-verification) + Buzz fleet (Rook, Chip, Warden — `m1-standards-ci` room). First session where the CI-config-verification standing item actually closed, and the first time fleet-built work went through a full dictate→fix→re-verify loop under explicit head-brain/execution-agent division ("You will dictate, monitor, and correct. The agents will do the work").

**What worked:**
- **The three-lane pattern generalized cleanly.** Python (#27) and Bash (#28) were built and merged earlier in the day; Go (#29) followed the same Surface/GROUND-TRUTH/TRAP/SHA-resolve-check shape with no structural rework needed.
- **The dictate→fix→re-verify loop, run for real over Buzz, worked as designed.** Claude found bugs against the real toolchain, wrote them up as precise, self-contained fix instructions (exact error text, exact root cause, exact suggested diff) posted to Rook via `buzz messages send`, Rook implemented and pushed, Claude re-verified independently rather than trusting the report. Two full round-trips on the Go lane (module-boundary build error, then the `install.sh` checksum bug) both landed correctly on the first attempt after the instructions were specific enough to act on without back-and-forth.
- **Real GitHub Actions runner caught a bug that no local re-verification — fleet or Claude — could have caught.** `golangci-lint`'s official `install.sh` has a genuine upstream bug for v2.12.2 (verifies the downloaded tarball against the release's `.sbom.json` checksum instead of the `.tar.gz`'s own), invisible locally because Claude's local re-verification used a pre-installed golangci-lint binary and never exercised the install script at all. This is the sharpest receipt yet for why the "real runner, not just local repro" gate in this repo's own doctrine can't be skipped — it directly validates the reason this whole CI-verification job exists.
- **`buzz-acp`'s restart-orphans-mentions behavior, already a known lesson, held up again in practice.** Rook's session was knocked offline mid-task by an unrelated Buzz Desktop app update on the fleet host; restarting it did not cause it to pick up the mention sent before the restart, so it had to be resent as a fresh message. Recognizing this immediately (rather than assuming Rook was just slow) avoided a stall.

**What didn't:**
- **Initial local re-verification gave false confidence.** Claude's first local pass on the Go lane (real Go, real golangci-lint binary, real gitleaks) passed clean and the PR was opened — the `install.sh` bug only surfaced once the actual GitHub Actions job ran. The lesson isn't "local verification is worthless," it's that verification needs to cover the *actual code path* the CI job runs, not just the tools it invokes — a pre-installed binary skips exactly the step (`install.sh`) that turned out to be broken.
- **A non-blocking fidelity gap survived through both fix rounds without being addressed:** the Go TRAP fixture's own comments claim `errcheck` is one of the two guardrails it exercises, but golangci-lint's default `errcheck` config doesn't flag blank-identifier-discarded errors on multi-value returns (`result, _ := f()`) — the fixture actually fails via `gofmt`/`nolintlint` instead. Flagged twice as non-blocking (TRAP still correctly distinguishes clean vs. violating), so it shipped un-actioned; worth a dedicated pass if the Go overlay's errcheck config is ever revisited.
- **Christopher explicitly corrected the working model mid-session:** after PR #27/#28 merge and the first Go bug report, he clarified the fleet — not Claude directly — should be doing the fix work, with Claude in a dictate/monitor/correct role. The first bug-fix round for the Bash overlay (PR #28) had been done by Claude patching the workflow file directly; this session's Go-lane bugs (module-boundary error, `install.sh` checksum bug) were instead routed back to Rook as written instructions every time, which is the pattern to keep.

**Standard impact:**
- [x] Actioned: CI config-verification standing item closed for Python/Bash/Go. `verify-templates.yml`, `verify-templates-bash.yml`, `verify-templates-go.yml` all merged to main (`ff1f7af`, `7110a28`, `556f25e`).
- [ ] Open: TS/Astro/Bun-ECS lanes not yet attempted — same pattern generalizes, no structural changes expected.
- [ ] Open: Go overlay's `errcheck` fidelity gap in the TRAP fixture (see above) — cosmetic, not urgent.
